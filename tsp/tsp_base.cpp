...
class Matrix {
public:
    Matrix(size_t N, std::size_t M) : m_data(N * M, T()), m_rows(N), m_cols(M) {}
    inline T* operator[](int i) { return &m_data[i * m_cols]; }
    inline T const* operator[](int i) const { return &m_data[i * m_cols]; }
...
uint32_t minDistance(const Matrix<uint32_t>& d) {
    size_t N = d.rows();
    uint32_t mn = numeric_limits<uint32_t>::max();
 ...
 */
inline void reverse(vector<uint16_t> &tour, size_t start, size_t end, vector<uint16_t>& position) {
    size_t N = tour.size();
 ...
 *
 * @param d Distance matrix.
 * @return d.rows() x K matrix where element i,j is the j:th nearest neighbor of city i.
 */
Matrix<uint16_t> createNeighborsMatrix(const Matrix<uint32_t>& d, size_t K) {
    size_t N = d.rows(), M = d.cols() - 1;
    K = min(M, K);
    ...
    while (!locallyOptimal) {
        ...
        for (u_i = 0, v_i = 1; u_i < N; ++u_i, ++v_i) {
            ...
            for (size_t k = 0; k < neighbor.cols(); ++k) {
                ...
                if (v == w || z == u) continue; // Skip adjacent edges.

                // d[u][w] + min is a lower bound on new length.
                // d[u][v] + max is an upper bound on old length.
                if (d[u][w] + min > d[u][v] + max) break; // Go to next edge uv.
...
vector<uint16_t> approximate(istream &in, const chrono::time_point<T>& deadline) {
    ...
    LOG("  distance: " << shortestTourLength);
